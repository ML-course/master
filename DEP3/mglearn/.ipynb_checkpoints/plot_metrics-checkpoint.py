...
def plot_decision_threshold():
    ...
    plot_2d_scores(svc, X_train, function="decision_function", alpha=.7,
                   ax=axes[0, 2], cm=ReBl)
    axes[0, 2].set_xlim(X_train[:, 0].min(), X_train[:, 0].max())
    ...
    axes[1, 1].contourf(line, np.linspace(-1.5, 1.5, 10), contour, linewidth=2, alpha=0.4, cmap=cm)
    discrete_scatter(X_train[mask, 0], np.zeros(bla), y_train[mask], ax=axes[1, 1])
    axes[1, 1].plot(line, np.array([0 for i in range(len(line))]), 'r:', linewidth=3) 
    axes[1, 1].set_xlim(X_train[:, 0].min(), X_train[:, 0].max())
    axes[1, 1].set_ylim(-1.5, 1.5)
    axes[1, 1].set_xticks(())
    axes[1, 1].set_yticks(np.arange(-1.4, 1.4, 0.2))
    ...
    axes[1, 2].contourf(line, np.linspace(-1.5, 1.5, 10), contour2, linewidth=2, alpha=0.4, cmap=cm)
    discrete_scatter(X_train[mask, 0], np.zeros(bla), y_train[mask], ax=axes[1, 2])
    axes[1, 2].plot(line, np.array([-0.8 for i in range(len(line))]), 'r:', linewidth=3) 
    axes[1, 2].plot(line, svc.decision_function(np.c_[line, 10 * np.ones(100)]), c='k')
    axes[1, 2].set_xlim(X_train[:, 0].min(), X_train[:, 0].max())
    axes[1, 2].set_ylim(-1.5, 1.5)
    axes[1, 2].set_xticks(())
    axes[1, 2].set_yticks(np.arange(-1.4, 1.4, 0.2))
